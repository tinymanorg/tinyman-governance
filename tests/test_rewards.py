...
from tinyman.governance.rewards.constants import CREATION_TIMESTAMP_KEY, VAULT_APP_ID_KEY, MANAGER_KEY, REWARD_HISTORY_COUNT_KEY, TINY_ASSET_ID_KEY
...
from tinyman.governance.rewards.transactions import prepare_claim_reward_transactions, prepare_init_transactions
...
class RewardsTestCase(VaultAppMixin, RewardsAppMixin, BaseTestCase):
    ...
    def test_claim_rewards(self):
        ...
        txn_group = prepare_claim_reward_transactions(
            ...
            account_power_period_start_index=0,
            account_power_period_end_index=0,
            total_power_period_start_index=2,
            total_power_period_end_index=3,
            has_reward_claim_sheet=False,
            suggested_params=self.sp,
        )
        txn_group.sign_with_private_key(self.user_address, self.user_sk)
        self.ledger.eval_transactions(txn_group.signed_transactions, block_timestamp=block_timestamp)
